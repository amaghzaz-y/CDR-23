<script lang="ts">
	...
	const distance_in_cm_to_steps = (steps: number) => {
		return (steps * 4000) / 18.8496;
	};
	const computeD = () => {
		let centimeters = 0;
		centimeters = distance_in_cm_to_steps(Distance);
		angle_1 = degrees_to_radians(angle_w - 90);
		angle_2 = degrees_to_radians(angle_w + 30);
		angle_3 = degrees_to_radians(angle_w + 150);
		Distance_M_1 = centimeters * Math.sin(angle_1);
		Distance_M_2 = centimeters * Math.sin(angle_2);
		Distance_M_3 = centimeters * Math.sin(angle_3);
	};
	const reset = () => {
		Distance = 0;
		angle_w = 0;
...
<div class="container">
	...
	<NumberInput label="Distance in cm" bind:value={Distance} step={100} />
